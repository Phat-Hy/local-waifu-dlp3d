...
async def inference_zero_shot(req: ZeroShotRequest):
    ...
    # Prefer clean 12s prompt sample if available
    try:
        clean_shiori = Path(prompt_path).parent / "shiori_prompt_12s.wav"
        if clean_shiori.exists():
            prompt_path = str(clean_shiori)
        else:
            import soundfile as sf
            info = sf.info(prompt_path)
            if info.duration > 20.0:
                trimmed_path = str(Path(prompt_path).parent / "trimmed_10s_prompt.wav")
                if not os.path.exists(trimmed_path):
                    data, sr = sf.read(prompt_path)
                    sf.write(trimmed_path, data[:sr * 10], sr)
